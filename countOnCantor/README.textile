...
p. One of the famous proofs of modern mathematics is Georg Cantor's demonstration 
...
the third term is 2/1, 
the fourth term is 3/1,
the fifth term is 2/2, 
and so on.

h2. Input and Output

p. You are to write a program that will read a list of numbers in the range from 1 
to 10ˆ7 and will print for each number the corresponding term in Cantor's 
enumeration as given below. No blank line should appear after the last number.

p. The input list contains a single number per line and will be terminated by 
end-of-file.
